...
export function rotaMaster(pathname: string): boolean {
  return NAV_MASTER.some((i) => pathname === i.to || pathname.startsWith(i.to + '/'))
}
...
export const ATALHOS: Record<Exclude<PerfilApp, 'aluno'>, { to: string; search?: string; label: string; texto: string }[]> = {
  ...
  superintendente: [
    ...
    { to: '/turmas', label: 'Turmas', texto: 'Presença e ranking de cada classe' },
